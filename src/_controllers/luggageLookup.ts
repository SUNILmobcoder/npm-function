...
const luggageLookup = (luggage: LuggageProps, operator: string, value: string[], key: string, source?: string) => {
  ...
  if (ANSWERS) {
    ...
    if (operator === 'Gt') {
      if (ANSWERS !== null || ANSWERS !== undefined) {
        return value.some((val) => ANSWERS?.some((num: string) => parseInt(val) > parseInt(num)));
      }
      return false;
    }
