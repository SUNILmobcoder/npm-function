...
const handleValidate = (props: QuestionProps, userResponse: string[]) => {
  ...
  if (Validation) {
    ...
    if (Type === 'TextField' || Type === 'MultiLineText') {
      ...
      if (Validation.pattern) {
        ...
        if (userResponse === null) return msg;

        // if validation didn't match
        if (!pattern.test(userResponse[0])) return msg;
      }
      ...
      if (Validation.max && Validation.min) {
        ...
        // if user haven't responsed
        if (userResponse === null) return minMsg;
